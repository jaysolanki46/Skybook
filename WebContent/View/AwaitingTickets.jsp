						...
						<%	
								...
								if(queryParameter == null || queryParameter.equals("0")) {
									...
									rs = st.executeQuery("select fup.*, dt.name as technician, d.name as dealer, u.name as user from " +
											...
											"INNER JOIN dealers as d ON dt.dealer = d.id " +
											"INNER JOIN users as u ON l.user = u.id " +
											"WHERE fup.is_completed = 0 and l.user =" + queryParameter + 
											" ORDER BY fup.follow_up_date, fup.follow_up_time");
										  ...
										  <div class="card-deck col mb-4" style="min-width:31rem;  max-width: 31rem;">
										    <div class="card" style="background-color: #f4f5f7;">
										      ...
										      <p>Created By: <%=rs.getString("user") %></p>
												<div style="margin-left: auto;">
													<a href=../View/Book.jsp?log=<%=rs.getString("log") %>><img alt="" width="20px" src="../IMAGES/info.svg"></a>
												</div>
										      </div>
										     </div>
										    </div>
